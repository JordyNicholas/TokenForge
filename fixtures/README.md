...
| `borderline-app/` | Precision (stress test): two legitimately large files (a hand-maintained `.ts` type surface, a real i18n config) still get `oversized` → `excluded`, and an unrelated `rules/` directory still gets prioritized for LLM enrichment. Documents two concrete heuristic gaps — see `docs/design/HEURISTICS_AUDIT.md` B1-B3. | `expected/borderline-app-totals.json` |
| `hybrid-eval-app/` | **Hybrid eval / presentation**: LLM-style paraphrased instruction sprawl across four provider files, load-bearing `docs/RULEBOOK.md`, lockfile + CI output excludes (~88% heuristic savings). Target for `npm run tokenforge:presentation-full`. | `expected/hybrid-eval-app-totals.json` |
| `instructions-app/` | Hybrid/LLM candidate routing: verbatim repeated paragraphs (legacy — prefer `hybrid-eval-app` for apply demos). | `expected/instructions-app-totals.json` |
...
| `active-session-app/` | The extension → CLI session signal (#137): a ~250 KB `locales/en.json` that is waste on most days and the whole task on the day someone is fixing a translation. Golden file pins the **baseline** (excluded) and records the with-signal totals beside it, so the `savedTokens` drop is visible as intended behaviour. Companion export lives in `expected/active-session-last-scan.json` — outside the fixture, or it would be scanned as part of the repo under test. | `expected/active-session-app-totals.json` |
...
| `heuristic-edge-app/` | Precision **and** recall for the #135 protection rules — the fixture where what's *absent* from the findings matters most: a 153 KB `openapi.json`, a `tsconfig.json`, and a generated GraphQL surface must stay unflagged, while 86 KB of recorded fixture data (under the flat 100 KB bar) must now be caught. Also holds two fake-credential files that must never reach an LLM enricher. See `docs/design/HEURISTICS_AUDIT.md` B11-B15. | `expected/heuristic-edge-app-totals.json` |
| `over-collapse-app/` | **Exclusion blast radius** (F14): every finding is a binary asset sitting beside living source — including one directly under `src/`. Proves collapse claims the deepest cluster (`assets/screenshots/**`) and never widens to a repo-root directory, that the generated pack matches no kept path, and that ~200-byte icons are excluded on the same footing as a 200 KB render. | `expected/over-collapse-app-totals.json` |
| `asset-dump-app/` | **Asset-tree density** (#301): three directories that hold nothing but a few hundred bytes of media each, plus one that has a `.ts` beside the icons. Proves a pure asset directory folds to a single `dir/**` finding, that the source veto is recursive, that a directory under the file floor stays per-file, and that a folded subtree still reconciles against the per-file totals. | `expected/asset-dump-app-totals.json` |
