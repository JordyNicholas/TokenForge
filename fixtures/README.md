...
| `hybrid-eval-app/` | **Hybrid eval / presentation**: LLM-style paraphrased instruction sprawl across four provider files, load-bearing `docs/RULEBOOK.md`, lockfile + CI output excludes (~88% heuristic savings). Target for `npm run tokenforge:presentation-hybrid`. | `expected/hybrid-eval-app-totals.json` |
