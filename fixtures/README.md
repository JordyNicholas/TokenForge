...
| `over-collapse-app/` | **Exclusion blast radius** (F14): every finding is an oversized binary sitting beside living source — including one directly under `src/`. Proves collapse claims the deepest cluster (`assets/screenshots/**`) and never widens to a repo-root directory, and that the generated pack matches no kept path. | `expected/over-collapse-app-totals.json` |
